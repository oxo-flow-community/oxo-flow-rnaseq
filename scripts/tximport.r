...
# (written by Lorena Pantano, later modified by Jonathan Manning, MIT).
# The Nextflow placeholders are replaced with an argparse-style CLI; the
# logic is otherwise the upstream script. Upstream runs this process with
# ext.prefix "salmon.merged" and no ext.args (column names keep their
# defaults transcript_id/gene_id/gene_name).
...
#' dimensions and row/column names of the counts data.
...
if (opt$quant_type == "rsem") {
    ...
    pattern <- ifelse(opt$quant_type == "kallisto", "abundance.tsv", "quant.sf")
    fns <- list.files(opt$quants_dir, pattern = pattern, recursive = TRUE, full.names = TRUE)
    names <- basename(dirname(fns))
    names(fns) <- names
    dropInfReps <- opt$quant_type == "kallisto"

    # Import transcript-level quantifications
    txi <- tximport(fns, type = opt$quant_type, txOut = TRUE, dropInfReps = dropInfReps)
}

# Read transcript and sample data
transcript_info <- read_transcript_info(opt$tx2gene, opt$tx_col, opt$gene_id_col, opt$gene_name_col)
...
done <- lapply(params, write_se_table, opt$prefix)

write.table(transcript_info$tx2gene_augmented,
    paste0(opt$prefix, ".tx2gene_augmented.tsv"),
    sep = "\t", quote = FALSE, row.names = FALSE)
